...
const formSchema = z.object({
  label: z.enum(['Office', 'Warehouse', 'Other'], { required_error: 'Please select a label.' }),
...
export function AddressDialog({ open, onOpenChange, onSave, addressToEdit, isLoading }: AddressDialogProps) {
  ...
  useEffect(() => {
    if (addressToEdit) {
      form.reset({
        ...addressToEdit,
        label: addressToEdit.label === 'Home' ? 'Office' : addressToEdit.label,
        coordinates: {
      ...
      form.reset({
        label: 'Office', name: '', phone: '', district: 'Dhaka',
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-4xl">
        <DialogHeader>
          <DialogTitle>{addressToEdit ? 'Edit Business Address' : 'Add Business Address'}</DialogTitle>
          <DialogDescription>
            Enter your business address details. Use the map to pinpoint your location.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
            ...
            <div className="space-y-4">
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <FormField control={form.control} name="name" render={({ field }) => (
                    <FormItem><FormLabel>Contact Person</FormLabel><FormControl><Input placeholder="Enter contact name" {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
                  <FormField control={form.control} name="phone" render={({ field }) => (
                    <FormItem><FormLabel>Business Phone</FormLabel><FormControl><Input placeholder="Enter phone number" {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
                </div>
                 <FormField control={form.control} name="label" render={({ field }) => (
                    <FormItem><FormLabel>Address Label</FormLabel>
                        ...
                        <SelectContent><SelectItem value="Office">Office</SelectItem><SelectItem value="Warehouse">Warehouse</SelectItem><SelectItem value="Other">Other</SelectItem></SelectContent>
                ...
                <FormField control={form.control} name="streetAddress" render={({ field }) => (
                    <FormItem><FormLabel>Street Address / Building</FormLabel><FormControl><Input placeholder="e.g., House 123, Road 45" {...field} /></FormControl><FormMessage /></FormItem>
                )} />
