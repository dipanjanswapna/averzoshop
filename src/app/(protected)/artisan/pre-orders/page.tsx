...
export default function ArtisanPreOrdersPage() {
  ...
  const artisanPreOrders = useMemo(() => {
    if (!orders || artisanProductIds.size === 0) return [];
    return orders
      .filter(order => 
        order.orderType === 'pre-order' &&
        order.items.some(item => artisanProductIds.has(item.productId))
      )
      .sort((a, b) => (b.createdAt?.toDate?.().getTime() || 0) - (a.createdAt?.toDate?.().getTime() || 0));
  }, [orders, artisanProductIds]);
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {isLoading ? renderSkeleton() : artisanPreOrders.length > 0 ? artisanPreOrders.map(order => (
